...
import { TransferQuickForm, TransferRevisionForm } from "@/components/forms/delivery-create-form";
...
export default async function TransfersPage({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string; status?: string; focus?: string }> }) {
  ...
  const canRevise = (direction: string) => direction === "plant_to_mine" ? ["workshop","warehouse_manager"].includes(profile.role) : profile.role === "mine_xsc";
