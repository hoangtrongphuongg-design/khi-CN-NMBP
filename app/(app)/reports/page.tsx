...
export default async function ReportsPage({
  ...
  return <div className="grid gap-5">
    ...
    <section id="van-chuyen"><Card className="overflow-hidden p-0"><div className="border-b border-[var(--border)] p-4 md:p-5"><div className="flex flex-wrap justify-between gap-3"><div><CardTitle>Cước vận chuyển</CardTitle><p className="mt-1 text-sm text-[var(--muted-foreground)]">1 Phiếu giao NCC = 1 chuyến = 1 cước. Phiếu trả vỏ cùng chuyến không phát sinh thêm cước.</p></div><div className="font-mono-data text-xl font-extrabold text-[var(--brand-deep)]">{formatCurrency(transportCost)}</div></div></div><div className="overflow-x-auto"><table className="mobile-card-table w-full text-sm"><thead className="bg-[var(--muted)]"><tr><th className="p-3 text-left">Ngày</th><th className="p-3 text-left">Phiếu giao / chuyến</th><th className="p-3 text-left">Địa điểm tính cước</th><th className="p-3 text-right">Đơn giá</th><th className="p-3 text-right">Thành tiền</th></tr></thead><tbody>{transportRows.map((r) => <tr key={r.trip_code} className="border-t border-[var(--border)]"><td data-label="Ngày" className="p-3">{shortDate(r.trip_date)}</td><td data-label="Phiếu giao / chuyến" className="p-3 font-mono-data">{r.delivery_code}</td><td data-label="Địa điểm tính cước" className="p-3">{r.trip_kind === "mine" ? "Mỏ Tà Thiết" : r.trip_kind === "co2_liquid" ? "CO₂ lỏng chuyên dụng" : "Nhà máy"}</td><td data-label="Đơn giá" className="p-3 text-right font-mono-data">{formatCurrency(r.unit_price)}</td><td data-label="Thành tiền" className="p-3 text-right font-mono-data font-extrabold">{formatCurrency(r.amount)}</td></tr>)}</tbody></table></div></Card></section>

    <div className="flex items-start gap-2 rounded-xl border border-[var(--border)] bg-white p-4 text-xs text-[var(--muted-foreground)]"><CheckCircle2 size={17} className="mt-0.5 shrink-0 text-[var(--success)]"/><span><strong>Nguyên tắc đối soát:</strong> mua khí lấy số thực nhận PHC; thuê vỏ lấy số vỏ cuối từng ngày; XL-45 tính bồn-ngày sau 15 ngày miễn phí và không tính ngày trả bồn; vận chuyển tính đúng 1 cước cho mỗi Phiếu giao NCC; trả vỏ đi cùng không tạo cước mới. Không phân bổ cước vận chuyển vào từng loại khí nếu chưa có quy tắc phân bổ chính thức.</span></div>
  </div>;
}
