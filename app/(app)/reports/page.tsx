...
export default async function ReportsPage({
  ...
  return <div className="grid gap-5">
    ...
    <section id="thue-vo"><Card className="overflow-hidden p-0">
      <div className="border-b border-[var(--border)] p-4 md:p-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Thuê vỏ theo từng loại khí · {periodLabel}</CardTitle><p className="mt-1 max-w-3xl text-sm text-[var(--muted-foreground)]">Mỗi ngày lấy số vỏ cuối ngày = nợ vỏ đầu kỳ + NCC giao - trả NCC. Tổng tiền = Σ (vỏ cuối ngày × đơn giá thuê của ngày đó).</p></div><div className="text-right"><div className="text-xs font-bold uppercase text-[var(--muted-foreground)]">Tổng thuê vỏ</div><div className="font-mono-data text-xl font-extrabold text-[var(--brand-deep)]">{formatCurrency(rentalCost)}</div></div></div><div className="mt-3 flex items-start gap-2 rounded-lg border border-[var(--border)] bg-[var(--paper)] p-3 text-xs text-[var(--muted-foreground)]"><Info size={16} className="mt-0.5 shrink-0 text-[var(--brand)]"/><span>Số dư đầu kỳ được kế thừa từ nợ vỏ năm trước. Từ 01/01/2026, chỉ giao/trả với NCC làm thay đổi tổng vỏ thuê; Kho ↔ Nhóm và Nhà máy ↔ Mỏ chỉ đổi vị trí giữ vỏ. Ngày tương lai không được tính.</span></div></div>
    ...
    <section id="xl45"><Card className="overflow-hidden p-0"><div className="border-b border-[var(--border)] p-4 md:p-5"><div className="flex flex-wrap justify-between gap-3"><div><CardTitle>Thuê / lưu bồn XL-45</CardTitle><p className="mt-1 text-sm text-[var(--muted-foreground)]">Miễn 15 ngày; từ ngày thứ 16 tính theo bồn-ngày. Ngày trả bồn cho NCC không tính phí.</p></div><div className="font-mono-data text-xl font-extrabold text-[var(--brand-deep)]">{formatCurrency(xl45Cost)}</div></div></div><div className="overflow-x-auto"><table className="mobile-card-table w-full text-sm"><thead className="bg-[var(--muted)]"><tr><th className="p-3 text-left">Loại</th><th className="p-3 text-right">Bồn-ngày tính phí</th><th className="p-3 text-right">Thành tiền</th></tr></thead><tbody>{xl45Summary.map((r) => <tr key={r.product_id} className="border-t border-[var(--border)]"><td data-label="Loại" className="p-3"><strong>{r.product_name}</strong><div className="text-xs text-[var(--muted-foreground)]">{r.product_code}</div></td><td data-label="Bồn-ngày tính phí" className="p-3 text-right font-mono-data">{formatNumber(r.bon_days)}</td><td data-label="Thành tiền" className="p-3 text-right font-mono-data font-extrabold">{formatCurrency(r.amount)}</td></tr>)}</tbody></table></div></Card></section>

    <section id="van-chuyen"><Card className="overflow-hidden p-0"><div className="border-b border-[var(--border)] p-4 md:p-5"><div className="flex flex-wrap justify-between gap-3"><div><CardTitle>Cước vận chuyển</CardTitle><p className="mt-1 text-sm text-[var(--muted-foreground)]">Cước lấy trực tiếp từ Chuyến vận chuyển. Cứ có một chuyến ghi nhận tại Nhà máy/Mỏ là tính một cước tương ứng; chuyến chỉ trả vỏ vẫn được tính nếu có phát sinh chuyến.</p></div><div className="font-mono-data text-xl font-extrabold text-[var(--brand-deep)]">{formatCurrency(transportCost)}</div></div></div><div className="overflow-x-auto"><table className="mobile-card-table w-full text-sm"><thead className="bg-[var(--muted)]"><tr><th className="p-3 text-left">Ngày</th><th className="p-3 text-left">Phát sinh / chuyến</th><th className="p-3 text-left">Địa điểm tính cước</th><th className="p-3 text-right">Đơn giá</th><th className="p-3 text-right">Thành tiền</th></tr></thead><tbody>{transportRows.map((r) => <tr key={r.trip_code} className="border-t border-[var(--border)]"><td data-label="Ngày" className="p-3">{shortDate(r.trip_date)}</td><td data-label="Phát sinh / chuyến" className="p-3"><div className="font-mono-data font-bold">{r.reference_code}</div>{r.reference_code !== r.trip_code ? <div className="mt-1 text-xs text-[var(--muted-foreground)] font-mono-data">{r.trip_code}</div> : null}</td><td data-label="Địa điểm tính cước" className="p-3">{r.trip_kind === "mine" ? "Mỏ Tà Thiết" : r.trip_kind === "co2_liquid" ? "CO₂ lỏng chuyên dụng" : "Nhà máy"}</td><td data-label="Đơn giá" className="p-3 text-right font-mono-data">{formatCurrency(r.unit_price)}</td><td data-label="Thành tiền" className="p-3 text-right font-mono-data font-extrabold">{formatCurrency(r.amount)}</td></tr>)}</tbody></table></div></Card></section>

    <div className="flex items-start gap-2 rounded-xl border border-[var(--border)] bg-white p-4 text-xs text-[var(--muted-foreground)]"><CheckCircle2 size={17} className="mt-0.5 shrink-0 text-[var(--success)]"/><span><strong>Nguyên tắc đối soát:</strong> mua khí lấy số thực nhận PHC; thuê vỏ lấy số vỏ cuối từng ngày; XL-45 tính bồn-ngày sau 15 ngày miễn phí và không tính ngày trả bồn; vận chuyển lấy Chuyến làm nguồn gốc: có phát sinh Nhà máy/Mỏ thì tính cước tương ứng; phiếu giao hoặc phiếu trả chỉ là chứng từ gắn với chuyến. Không phân bổ cước vận chuyển vào từng loại khí nếu chưa có quy tắc phân bổ chính thức.</span></div>
  </div>;
}
