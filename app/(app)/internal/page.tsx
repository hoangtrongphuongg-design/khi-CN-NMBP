    ...
    <Card className="overflow-hidden p-0"><div className="border-b border-[var(--border)] p-4 md:p-5"><div className="flex items-center justify-between gap-3"><CardTitle>Danh sách phiếu</CardTitle><span className="rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-bold text-[var(--brand)]">{requests.length} phiếu gần nhất</span></div></div><div className="grid gap-3 p-3 md:p-5">
      {requests.map((r:any)=>{const items=Array.isArray(r.items)?r.items:[];const canExecute=canExecuteWarehouse(profile)&&(r.request_type==="borrow"?(r.status==="approved"||(!office&&r.status==="pending")):r.status==="pending");return <article key={r.id} className="rounded-2xl border border-[var(--border)] bg-white p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {r.status==="feedback"&&profile.role==="storekeeper"?<form action="/api/internal" method="post" className="w-full rounded-2xl border border-red-200 bg-red-50/40 p-3"><input type="hidden" name="action" value="revise_after_feedback"/><input type="hidden" name="request_id" value={r.id}/><input type="hidden" name="item_ids" value={items.map((x:any)=>x.id).join(",")}/><div className="mb-1 text-sm font-extrabold text-[var(--danger)]">Chỉnh theo phản hồi</div><div className="mb-3 text-xs text-[var(--muted-foreground)]">Tồn đã cập nhật ở lần xử lý trước. Khi gửi lại, hệ thống chỉ điều chỉnh phần chênh lệch và lưu lịch sử.</div><div className="grid gap-2">{items.map((item:any)=><div key={item.id} className="grid gap-2 rounded-xl bg-white p-3 md:grid-cols-[1.4fr_.8fr_.8fr] md:items-end"><div><div className="font-bold">{item.product_name}</div><div className="text-xs text-[var(--muted-foreground)]">Yêu cầu {formatNumber(item.requested_qty)} {item.unit}</div></div><label className="grid gap-1 text-xs font-bold">SL thực tế<Input name={`actual_${item.id}`} type="text" inputMode="numeric" pattern="[0-9]*" autoComplete="off" defaultValue={item.actual_qty ?? ""} required/></label>{r.request_type==="return"?<label className="grid gap-1 text-xs font-bold">Tình trạng<Select name={`bucket_${item.id}`} defaultValue={item.return_bucket || "empty"}><option value="empty">Rỗng</option><option value="full">Đầy</option></Select></label>:<div className="rounded-lg bg-blue-50 p-2 text-xs text-[var(--brand)]">Nhập lại số thực tế đúng sau phản hồi.</div>}</div>)}</div><Button type="submit" className="mt-3">Cập nhật & gửi lại duyệt</Button></form>:null}
