...
export default async function InternalPage({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  const [requests, office, quickProducts] = await Promise.all([listInternalRequests(profile),isOfficeHours(),isGroupUser ? getGroupQuickData(profile) : Promise.resolve([])]);
  const canCreate = canCreateGroupRequest(profile) && profile.location_code !== "MINE";
  const active = requests.filter((r:any)=>!["completed","cancelled","rejected"].includes(r.status));
  const count = (type:string)=>active.filter((r:any)=>r.request_type===type).length;
  const discrepancy = requests.filter((r:any)=>Array.isArray(r.items)&&r.items.some((i:any)=>i.actual_qty!=null&&Number(i.actual_qty)!==Number(i.requested_qty))).length;
