...
export default async function DeliveriesPage({ searchParams }: { searchParams: Promise<{ error?: string; ok?: string }> }) {
  ...
  return <div className="grid gap-5">
    <div><h1 className="font-display m-0 text-2xl text-[var(--brand-deep)]">Giao nhận NCC</h1><p className="mt-1 text-sm text-[var(--muted-foreground)]">NCC tạo Phiếu giao → XSC xác nhận số thực nhận theo địa điểm → PHC xác nhận bước cuối. Phiếu trả vỏ có thể liên kết cùng mã chuyến để chỉ tính 1 cước.</p></div>
    ...
    <Card className="overflow-hidden p-0"><div className="border-b border-[var(--border)] p-4 md:p-5"><CardTitle>Phiếu giao gần nhất</CardTitle></div><div className="grid gap-3 p-3 md:p-5">
      {deliveries.map((d: any) => <article key={d.id} className="rounded-xl border border-[var(--border)] bg-white p-4">
        ...
        <div className="mt-4 grid gap-2">{(d.items || []).map((item: any) => {
          ...
          const canFeedback = canFeedbackDelivery(profile) && item.status !== "confirmed";
          ...
          {(canConfirmNow || canFeedback) ? <form action="/api/deliveries" method="post" className="mt-3 grid gap-2 sm:grid-cols-[140px_1fr_auto_auto] sm:items-end"><input type="hidden" name="item_id" value={item.id} /><label className="grid gap-1 text-xs font-bold">SL thực nhận<Input name="actual_qty" type="number" min="0" step="0.001" defaultValue={item.confirmed_qty ?? item.declared_qty} /></label><label className="grid gap-1 text-xs font-bold">Phản hồi<Input name="feedback" placeholder="Không bắt buộc" /></label>{canFeedback ? <Button name="action" value="feedback_delivery_item" variant="secondary">Phản hồi</Button> : <span/>}{canConfirmNow ? <Button name="action" value="confirm_delivery_item">XSC xác nhận</Button> : null}</form> : null}
          {item.feedback ? <div className="mt-2 text-xs font-semibold text-[#B91C1C]">{item.feedback}</div> : null}</div>;
        })}</div>
        {d.status === "phc_pending" && canFinalizePhcDelivery(profile) ? <form action="/api/deliveries" method="post" className="mt-4 flex justify-end border-t border-[var(--border)] pt-4"><input type="hidden" name="delivery_id" value={d.id} /><Button name="action" value="finalize_delivery_phc">PHC xác nhận hoàn tất</Button></form> : null}
      </article>)}
