    ...
    <section className={compact ? "grid gap-4" : "grid gap-4 md:gap-5"}>
      <div className="overflow-hidden rounded-2xl border border-[var(--border)] bg-white">
        <div className="group-hero flex items-center justify-between gap-3 px-5 py-5 text-white md:px-6">
          <div>
            <div className="text-xs font-bold uppercase tracking-[.16em] text-white/70">Quản lý khí NMBP</div>
            <h1 className="m-0 mt-1 text-2xl font-bold md:text-3xl">{groupName}</h1>
        ...
        <div className="grid gap-5 p-4 md:p-6">
          <section>
            <div className="mb-3 flex items-center gap-2"><Boxes size={19} className="text-[var(--brand)]"/><h2 className="m-0 text-lg font-bold">Số chai tại nhóm</h2></div>
            {visibleGroup.length ? (
              <div className="grid grid-cols-3 gap-2.5 sm:gap-3 lg:grid-cols-5">
                {visibleGroup.map((p) => { const t = tone(p.code); return <div key={p.id} className="rounded-2xl border border-[var(--border)] bg-white p-2.5 shadow-sm sm:p-3.5 md:p-4">
                  <div className="flex items-center gap-3"><div className={`grid h-9 w-9 place-items-center rounded-xl sm:h-10 sm:w-10 ${t.icon}`}><Boxes size={22}/></div><div className="min-w-0"><div className="truncate text-[13px] font-semibold sm:text-sm">{p.name}</div><div className={`font-mono-data mt-1 text-2xl font-bold ${t.number}`}>{formatNumber(p.groupQty)} <span className="font-sans text-xs font-semibold text-[var(--muted-foreground)]">{p.unit}</span></div></div></div>
                </div>; })}
              </div>
          ...
          <section>
            <div className="mb-3 flex items-center gap-2"><Warehouse size={19} className="text-[var(--brand)]"/><h2 className="m-0 text-lg font-bold">Tồn Kho Hậu cần</h2></div>
            {visibleWarehouse.length ? <div className="grid gap-2.5 md:grid-cols-2 xl:grid-cols-3">
              {visibleWarehouse.map((p) => { const t = tone(p.code); return <div key={p.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-3 rounded-2xl border border-[var(--border)] bg-white px-4 py-3 shadow-sm">
                ...
                <div className="min-w-16 text-center"><div className="text-[11px] font-bold uppercase tracking-wide text-[var(--success)]">Đầy</div><div className={`font-mono-data text-xl font-bold ${p.warehouseFull > 0 ? "text-[var(--success)]" : "text-[var(--muted-foreground)]"}`}>{formatNumber(p.warehouseFull)}</div></div>
                <div className="min-w-16 border-l border-[var(--border)] pl-3 text-center"><div className="text-[11px] font-bold uppercase tracking-wide text-[var(--muted-foreground)]">Rỗng</div><div className="font-mono-data text-xl font-bold text-[var(--neutral)]">{formatNumber(p.warehouseEmpty)}</div></div>
              </div>; })}
      ...
      {mode ? <div className="fixed inset-0 z-[80] flex items-end justify-center bg-slate-950/35 p-0 md:items-center md:p-5" role="presentation">
        <div role="dialog" aria-modal="true" aria-labelledby="quick-sheet-title" className="max-h-[88vh] w-full overflow-y-auto rounded-t-[28px] border border-[var(--border)] bg-white p-4 shadow-2xl md:max-w-3xl md:rounded-2xl md:p-5">
          ...
          <div className="flex items-start justify-between gap-3">
            <div><h2 id="quick-sheet-title" className="m-0 text-xl font-bold">Tạo phiếu {MODE_LABEL[mode]}</h2><p className="mb-0 mt-1 text-sm text-[var(--muted-foreground)]">Có thể yêu cầu nhiều loại khí trong một phiếu.</p></div>
          ...
          <button type="button" onClick={askSubmit} disabled={submitting || !items.length} className="mt-4 flex min-h-13 w-full items-center justify-center gap-2 rounded-2xl bg-[var(--brand)] px-4 text-base font-bold text-white hover:bg-[var(--brand-hover)] disabled:opacity-50"><Send size={20}/>{submitting ? "Đang gửi..." : `Gửi yêu cầu ${MODE_LABEL[mode]}`}</button>
        </div>
      </div> : null}
      ...
      {discardOpen ? <div className="fixed inset-0 z-[96] grid place-items-center bg-slate-950/45 p-4"><div className="w-full max-w-sm rounded-2xl border border-[var(--border)] bg-white p-5 shadow-2xl"><div className="flex gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-amber-50 text-[var(--warning)]"><AlertTriangle size={20}/></div><div><h3 className="m-0 text-base font-bold">Bỏ nội dung đang nhập?</h3><p className="mb-0 mt-2 text-sm leading-6 text-[var(--muted-foreground)]">Các dòng khí và ghi chú chưa gửi sẽ bị bỏ.</p></div></div><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setDiscardOpen(false)} className="min-h-11 rounded-xl border border-[var(--border)] font-bold">Tiếp tục nhập</button><button type="button" onClick={resetSheet} className="min-h-11 rounded-xl bg-[var(--danger)] font-bold text-white">Bỏ nội dung</button></div></div></div> : null}

      {removeIndex != null ? <div className="fixed inset-0 z-[97] grid place-items-center bg-slate-950/45 p-4"><div className="w-full max-w-sm rounded-2xl border border-[var(--border)] bg-white p-5 shadow-2xl"><div className="flex gap-3"><div className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-red-50 text-[var(--danger)]"><Trash2 size={20}/></div><div><h3 className="m-0 text-base font-bold">Xóa loại khí khỏi phiếu?</h3><p className="mb-0 mt-2 text-sm leading-6 text-[var(--muted-foreground)]">Dòng này sẽ được bỏ khỏi yêu cầu đang nhập.</p></div></div><div className="mt-5 grid grid-cols-2 gap-2"><button type="button" onClick={() => setRemoveIndex(null)} className="min-h-11 rounded-xl border border-[var(--border)] font-bold">Hủy</button><button type="button" onClick={confirmRemoveItem} className="min-h-11 rounded-xl bg-[var(--danger)] font-bold text-white">Xóa dòng</button></div></div></div> : null}

      {toast ? <div className="fixed inset-x-4 top-4 z-[110] mx-auto flex max-w-md items-center gap-3 rounded-xl border border-[var(--border)] bg-white p-3 shadow-lg md:left-auto md:right-5 md:mx-0"><div className={`grid h-9 w-9 place-items-center rounded-full ${toast.tone === "success" ? "bg-green-50 text-[var(--success)]" : "bg-red-50 text-[var(--danger)]"}`}>{toast.tone === "success" ? <CheckCircle2 size={20}/> : <AlertTriangle size={20}/>}</div><div className="min-w-0 flex-1"><div className="text-xs font-bold uppercase tracking-wide text-[var(--muted-foreground)]">{toast.tone === "success" ? "Thành công" : "Không thành công"}</div><div className="mt-0.5 text-sm font-semibold">{toast.message}</div></div><button type="button" onClick={() => setToast(null)} className="rounded-lg p-1.5 hover:bg-[var(--muted)]"><X size={16}/></button></div> : null}
    </section>
  );
}
